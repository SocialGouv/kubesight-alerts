...
import { localKubeCache } from "@/lib/kube-watcher"
...
export function getLogsUrl(
  ...
  return process.env.GRAFANA_URL_LOGS_DEPLOYMENT?.replace(
    ...
    .replace("{{namespace}}", workload.metadata.namespace)
      ...
      const cleanedDeployments = _.chain(pods)
        ...
        .map((rs, deployName) => {
          ...
          return {
            ...
            replicasets: rs,
            raw: rawDeployment,
            logsUrl: rawDeployment
              ? getLogsUrl(cluster, rawDeployment)
              : undefined,
          }
        })
        .value()
      ...
      const cleanedCronjobs = _.chain(jobs)
        ...
        .map((cronJobs, cronjobName) => {
          ...
          return {
            ...
            jobs: cleanedJobs,
            raw: rawCronjob,
            logsUrl: rawCronjob ? getLogsUrl(cluster, rawCronjob) : undefined,
          }
        })
        .value()
...
// async function getCnpgStorageStats({ cluster }: { cluster: RawCnpgCluster }) {
//   const { stdout } =
