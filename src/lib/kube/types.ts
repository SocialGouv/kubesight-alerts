...
export const cnpgClusterSchema = z.object({
  metadata: z.object({
    name: z.string(),
    namespace: z.string(),
...
export type RawCnpgCluster = z.infer<typeof cnpgClusterSchema>
...
export type CnpgCluster = RawCnpgCluster & {
  // storageStats: {
  //   total: string
  //   used: string
  //   percentUsed: string
...
export function getInstances(cluster: CnpgCluster) {
...
export function getLastSuccessfulJob(cronjob: Cronjob): Job | undefined {
...
export function getJobsAfterlastSuccessful(cronjob: Cronjob): Job[] {
  const lastSuccess = getLastSuccessfulJob(cronjob)
...
export function getLastErrorJobsAfterLastSuccessful(cronjob: Cronjob): Job[] {
  return getJobsAfterlastSuccessful(cronjob)
    .filter((job) => job.raw.status.succeeded !== 1)
    .sort((job) => job.raw.status.completionTime?.getTime() || 0)
}
...
export function getCronjobStatus(cronjob: Cronjob): Status {
  ...
  if (
    dayjs(lastSuccess.raw.status.completionTime) <
    dayjs().subtract(24 + 12, "hour") // for daily cron
  ) {
    return "error"
  }

  const jobsAfterLastSuccessfull = getJobsAfterlastSuccessful(cronjob)
...
export function getCnpgClusterStatus(cluster: CnpgCluster): Status {
...
export function getBaseBackupStatus(cluster: CnpgCluster): Status {
...
export function getCnpgClusterArchivingStatus(cluster: CnpgCluster): Status {
...
export function getNamespaceStatus(namespace: Namespace): Status {
  if (
    hasError(namespace.cnpgClusters, getCnpgClusterStatus) ||
